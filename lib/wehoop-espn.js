...
const WNBA_LIVE_SCOREBOARD = 'https://cdn.wnba.com/static/json/liveData/scoreboard/todaysScoreboard_10.json';
const WNBA_SEASON_SCHEDULE = 'https://cdn.wnba.com/static/json/staticData/scheduleLeagueV2_10.json';
...
function normalizeWnbaLiveEvent(game = {}) {
  const home = game.homeTeam || {};
  const away = game.awayTeam || {};
  const fullName = team => [team.teamCity, team.teamName].filter(Boolean).join(' ').trim();
  const state = Number(game.gameStatus) === 2 ? 'in' : Number(game.gameStatus) === 3 ? 'post' : 'pre';
  const completed = Number(game.gameStatus) === 3;
  const parts = isoParts(game.gameTimeUTC || game.gameEt || '');
  return {
    idEvent: String(game.gameId || ''),
    strTimestamp: game.gameTimeUTC || game.gameEt || '',
    dateEvent: parts.date,
    strTime: parts.time,
    idHomeTeam: String(home.teamId || ''),
    strHomeTeam: fullName(home),
    idAwayTeam: String(away.teamId || ''),
    strAwayTeam: fullName(away),
    intHomeScore: numberValue(home.score),
    intAwayScore: numberValue(away.score),
    strVenue: game.arena?.arenaName || game.arenaName || '',
    strStatus: game.gameStatusText || (completed ? 'Final' : 'Scheduled'),
    strState: state,
    intPeriod: numberValue(game.period),
    strClock: game.gameClock || '',
    strBroadcasts: [],
...
async function getWnbaLiveScoreboard() {
  const body = await fetchJson(WNBA_LIVE_SCOREBOARD);
  const scoreboardEvents = (Array.isArray(body?.scoreboard?.games) ? body.scoreboard.games : [])
    .map(normalizeWnbaLiveEvent)
    .filter(event => event.idEvent && event.strHomeTeam && event.strAwayTeam);
  if (scoreboardEvents.some(event => event.strState === 'in')) return scoreboardEvents;

  // Some server regions roll the "today" scoreboard to the next calendar day
  // before games on the U.S. West Coast have ended. The official season file
  // still marks those games live, so use their IDs to request official boxscores.
  const scheduleBody = await fetchJson(WNBA_SEASON_SCHEDULE);
  const liveScheduleGames = (Array.isArray(scheduleBody?.leagueSchedule?.gameDates) ? scheduleBody.leagueSchedule.gameDates : [])
    .flatMap(item => Array.isArray(item.games) ? item.games : [])
    .filter(game => Number(game.gameStatus) === 2 && game.gameId);
  if (!liveScheduleGames.length) return scoreboardEvents;
  const boxscores = await Promise.allSettled(liveScheduleGames.map(game =>
    fetchJson(`https://cdn.wnba.com/static/json/liveData/boxscore/boxscore_${encodeURIComponent(game.gameId)}.json`)
  ));
  return boxscores
    .filter(result => result.status === 'fulfilled' && result.value?.game)
    .map(result => normalizeWnbaLiveEvent(result.value.game))
    .filter(event => event.idEvent && event.strHomeTeam && event.strAwayTeam);
}
