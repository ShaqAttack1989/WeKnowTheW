...
async function renderStartingFive(){
  ...
  const cards=featuredPlayers.map(item=>{
    ...
    const media=photo?`<img src="${sSafe(photo)}" alt="${sSafe(item.name)}" loading="lazy" decoding="async" referrerpolicy="no-referrer" onerror="this.remove()">`:`<span>${sSafe(item.name.split(' ').map(part=>part[0]).join('').slice(0,2))}</span>`;
