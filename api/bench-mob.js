function text(value=''){return String(value??'').replace(/<[^>]+>/g,' ').replace(/&amp;/g,'&').replace(/&#39;|&#x27;/g,"'").replace(/&quot;/g,'"').replace(/&nbsp;/g,' ').replace(/\s+/g,' ').trim();}
...
function parsePerGameMarkdown(raw=''){
  ...
  for(const line of String(raw).split(/\r?\n/)){
    ...
    if(h.includes('player')&&h.includes('gs')&&h.includes('pts')&&h.includes('ast')){
      const gs=lastIndex(h,'gs'), g=lastIndex(h,'g'), mp=lastIndex(h,'mp');
      ix={player:h.indexOf('player'),team:h.indexOf('team'),pos:h.indexOf('pos'),g,gs,mp,
  ...
  for(const line of String(raw).split(/\r?\n/)){
    ...
    if(h.includes('player')&&h.includes('per')&&(h.includes('ts%')||h.includes('ts'))){
      ix={player:h.indexOf('player'),team:h.indexOf('team'),per:h.indexOf('per'),ts:h.includes('ts%')?h.indexOf('ts%'):h.indexOf('ts'),
        ...
        ortg:h.indexOf('ortg'),drtg:h.indexOf('drtg'),ows:h.indexOf('ows'),dws:h.indexOf('dws'),ws:h.indexOf('ws'),ws40:h.indexOf('ws/40')};continue;
    }
    if(!ix||c.every(v=>/^[-: ]+$/.test(v)))continue;
    const name=text(c[ix.player]);if(!name||name.toLowerCase()==='player')continue;
...
async function fetchText(url,headers={}){const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),12000);try{const r=await fetch(url,{headers,signal:controller.signal});if(!r.ok)throw new Error(`${url} returned ${r.status}`);return await r.text();}finally{clearTimeout(timer);}}
async function sourceRows(season,suffix,markdownParser,htmlParser){
  ...
  if(rows.length<20){try{rows=htmlParser(await fetchText(source,{Accept:'text/html','User-Agent':'Mozilla/5.0 (compatible; WeKnowTheW/1.0)'}));if(rows.length<20)throw new Error(`direct parsed ${rows.length}`);}catch(e){errors.push(e.message);}}
...
function chooseBest(rows=[]){
  const best=new Map();
  for(const row of rows){const k=key(row.name);if(!k)continue;const combined=/^\d+TM$/i.test(String(row.team||''));const score=(combined?1e9:0)+safe(row.g)*1e5+safe(row.mpg)*100;const cur=best.get(k);if(!cur||score>cur.score)best.set(k,{row,score});}
...
function weightedScores(candidates,metrics){const pools={};for(const m of metrics)pools[m.key]=candidates.map(p=>m.value(p)).filter(Number.isFinite);return candidates.map(p=>{let score=0,total=0;for(const m of metrics){const v=m.value(p);if(!Number.isFinite(v))continue;score+=percentile(v,pools[m.key],m.higher!==false)*m.weight;total+=m.weight;}return {...p,roleScore:total?score/total:0};}).sort((a,b)=>b.roleScore-a.roleScore);}
...
function why(role,p){
  ...
  };return base[role.id]||role.subtitle;
}
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    let players=pgRows.map(p=>({...p,...(advBy.get(key(p.name))||{}),name:p.name,team:p.team,position:p.position}));
    const maxG=Math.max(...players.map(p=>safe(p.g)),1),minGames=Math.max(8,Math.floor(maxG*.33));
    players=players.filter(p=>safe(p.g)>=minGames&&safe(p.mpg)>=8&&!STARTING_FIVE.has(key(p.name)));
    ...
    for(const role of roles){
      ...
      const ranked=weightedScores(pool,role.metrics);const pick=ranked[0];if(!pick)continue;used.add(key(pick.name));
      picks.push({role:role.title,roleId:role.id,subtitle:role.subtitle,name:pick.name,team:pick.team,position:pick.position,score:Number((pick.roleScore*100).toFixed(1)),statLine:roleLine(role,pick),why:why(role,pick),stats:{g:pick.g,gs:pick.gs,mpg:pick.mpg,pts:pick.pts,trb:pick.trb,ast:pick.ast,stl:pick.stl,blk:pick.blk,tov:pick.tov,three:pick.three,threePct:pick.threePct,per:pick.per,tsPct:pick.tsPct,ws40:pick.ws40}});
    }
    return res.status(200).json({season,week:week||null,updatedAt:new Date().toISOString(),minGames,picks,methodology:'Season-to-date WNBA per-game and advanced metrics. Players must clear a games/minutes sample, Shak’s Starting Five is excluded, and one player can hold only one Bench Mob role.',sources:[pg.source,adv.source]});
