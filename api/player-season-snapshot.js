...
function parsePerGameMarkdown(raw=''){
  ...
  for(const line of String(raw).split(/\r?\n/)){
    ...
    if(h.includes('player')&&h.includes('pts')&&h.includes('ast')&&h.includes('trb')){
      ix={player:h.indexOf('player'),team:h.indexOf('team'),pos:h.indexOf('pos'),g:lastIndex(h,'g'),mp:lastIndex(h,'mp'),trb:h.indexOf('trb'),ast:h.indexOf('ast'),stl:h.indexOf('stl'),blk:h.indexOf('blk'),tov:h.indexOf('tov'),pts:h.indexOf('pts')};
      continue;
    }
...
function parsePerGameHtml(raw=''){
  ...
  for(const m of String(raw).matchAll(/<tr[^>]*>([\s\S]*?)<\/tr>/gi)){
    ...
    const item={name,team:text(cellByStat(r,'team_name_abbr')||cellByStat(r,'team_id')),position:text(cellByStat(r,'pos')),g:num(text(cellByStat(r,'g'))),mpg:num(text(cellByStat(r,'mp_per_g'))),trb:num(text(cellByStat(r,'trb_per_g'))),ast:num(text(cellByStat(r,'ast_per_g'))),stl:num(text(cellByStat(r,'stl_per_g'))),blk:num(text(cellByStat(r,'blk_per_g'))),tov:num(text(cellByStat(r,'tov_per_g'))),pts:num(text(cellByStat(r,'pts_per_g')))};
...
function parseAdvancedMarkdown(raw=''){
  ...
  for(const line of String(raw).split(/\r?\n/)){
    ...
    if(h.includes('player')&&h.includes('per')&&(h.includes('ts%')||h.includes('ts'))){
      ix={player:h.indexOf('player'),team:h.indexOf('team'),per:h.indexOf('per'),ts:h.includes('ts%')?h.indexOf('ts%'):h.indexOf('ts'),ws40:h.indexOf('ws/40')};continue;
    }
    if(!ix||c.every(v=>/^[-: ]+$/.test(v)))continue;
    const name=text(c[ix.player]);if(!name||name.toLowerCase()==='player')continue;
...
async function fetchText(url,headers={}){
  ...
  try{const response=await fetch(url,{headers,signal:controller.signal});if(!response.ok)throw new Error(`${url} returned ${response.status}`);return await response.text();}
...
function chooseBest(rows=[]){
  ...
  for(const row of rows){
    const playerKey=key(row.name);if(!playerKey)continue;
    const combined=/^\d+TM$/i.test(String(row.team||''));
    const score=(combined?1e9:0)+safe(row.g)*1e5+safe(row.mpg)*100;
    const current=best.get(playerKey);if(!current||score>current.score)best.set(playerKey,{row,score});
  }
...
function percentile(value,values,higher=true){
  ...
  let count=0;for(const item of clean){if(item<=value)count++;}
  const p=count/clean.length;return higher?p:1-p+1/clean.length;
}
function letter(score){
  if(!Number.isFinite(score))return 'NR';
  if(score>=95)return 'A+';if(score>=90)return 'A';if(score>=87)return 'A-';if(score>=84)return 'B+';if(score>=80)return 'B';if(score>=77)return 'B-';if(score>=74)return 'C+';if(score>=70)return 'C';if(score>=67)return 'C-';if(score>=64)return 'D+';if(score>=60)return 'D';return 'F';
}
function metricPools(players){
  const defs=[['per',p=>p.per,true,.24],['ts',p=>p.tsPct,true,.14],['pts',p=>p.pts,true,.18],['ast',p=>p.ast,true,.10],['trb',p=>p.trb,true,.10],['stocks',p=>safe(p.stl)+safe(p.blk),true,.08],['ws40',p=>p.ws40,true,.10],['astTo',p=>p.ast===null?null:safe(p.ast)/Math.max(safe(p.tov),.5),true,.06]];
  const pools={};defs.forEach(([id,get])=>pools[id]=players.map(get).filter(Number.isFinite));return {defs,pools};
}
function gradePlayer(player,defs,pools){
  let weighted=0,total=0;
  for(const [id,get,higher,weight] of defs){const value=get(player);const p=percentile(value,pools[id],higher);if(p===null)continue;weighted+=p*weight;total+=weight;}
...
module.exports=async function handler(req,res){
  ...
  try{
    const [pg,adv]=await Promise.all([sourceRows(season,'per_game',parsePerGameMarkdown,parsePerGameHtml),sourceRows(season,'advanced',parseAdvancedMarkdown,parseAdvancedHtml)]);
    const perGame=chooseBest(pg.rows),advanced=chooseBest(adv.rows),advancedMap=new Map(advanced.map(row=>[key(row.name),row]));
    ...
    return res.status(200).json({season,updatedAt:new Date().toISOString(),source:'Basketball-Reference',sourceUrls:[pg.source,adv.source],minGames,methodology:'Same We Know the W weighted league-relative grade used for active players, paired with that season’s per-game and advanced statistics.',players});
