(()=>{
  ...
  function collectStrings(value,out=[]){
    ...
    if(typeof value==='object')Object.entries(value).forEach(([key,item])=>{if(!['sources','url','teams','players'].includes(key))collectStrings(item,out);});
    return out;
  }

  function mentions(post,names=[]){
    const explicitPlayers=Array.isArray(post.players)?post.players:[];
    const hay=` ${norm(collectStrings(post).join(' '))} `;
    return names.filter(name=>{
      const needle=norm(name);
      return explicitPlayers.some(player=>norm(player)===needle)||(needle.length>3&&hay.includes(` ${needle} `));
    });
  }

  function explicitlyTaggedToTeam(post){
    return (Array.isArray(post.teams)?post.teams:[]).some(name=>norm(name)===norm(teamData.name));
  }
  ...
  function updateEmptyState(){
    ...
    if(copy)copy.textContent='Player mentions and team stories from We Know the W are shown above. Roster and availability alerts will appear here when posted.';
  }
  ...
  async function loadStories(){
    ...
    try{
      ...
      const [weeklyResult,specialResult,breakingResult]=await Promise.allSettled([
        ...
        fetch(`/snack-shak-breaking.json?teamMentions=${stamp}`,{headers:{Accept:'application/json','Cache-Control':'no-cache'},cache:'no-store'}).then(r=>r.ok?r.json():{})
      ]);
      ...
      const breaking=breakingResult.status==='fulfilled'&&Array.isArray(breakingResult.value.posts)?breakingResult.value.posts:[];
      const combined=[...breaking,...specials,...weekly].filter(post=>post&&post.slug&&post.published);
      stories=combined.map(post=>({post,players:mentions(post,names)})).filter(item=>item.players.length||explicitlyTaggedToTeam(item.post)).sort((a,b)=>String(b.post.published).localeCompare(String(a.post.published))||Number(b.post.priority||0)-Number(a.post.priority||0)).slice(0,4);
      ready=true;
      appendStories();
    }catch{ready=true;}
  }
