...
function clubSlug(v=''){return String(v).toLowerCase().replace(/bc$/,'').trim().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');}
function clubLogo(v=''){return `/api/league-team-image?league=unrivaled&key=${encodeURIComponent(clubSlug(v))}`;}
