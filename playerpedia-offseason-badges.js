(()=>{
  ...
  function buildMaps(payload={}){
    ...
    tauihi=new Map((payload?.tauihi?.players||[]).map(([name,team,label])=>[key(name),{team,label}]));
  ...
  function affiliationsFor(name){
    ...
    if(tauihi.has(k)){const entry=tauihi.get(k);items.push({league:'tauihi',team:entry.team,label:entry.label||'2026 Tauihi season',logo:''});}
    return items;
  }
  ...
  fetch(`/pro-offseason-affiliations.json?v=20260920-hvl-tauihi-v1&cb=${Date.now()}`,{headers:{Accept:'application/json','Cache-Control':'no-cache'},cache:'no-store'})
