...
function apiMedia(player,name){
  ...
  return {
    ...
    caption:`${name} · current roster artwork`,
    creator:'TheSportsDB',
    source:'TheSportsDB',
    sourceUrl:player.id?`https://www.thesportsdb.com/player/${encodeURIComponent(player.id)}`:'https://www.thesportsdb.com/',
    license:cc||'API artwork',
    licenseUrl:'https://www.thesportsdb.com/docs_terms_of_use.php',
    resolvedBy:'current-roster-api'
  };
}
...
async function hydrateAvatar(avatar){
  ...
  avatar.title=media.resolvedBy==='current-roster-api'?'Current roster artwork via TheSportsDB':'Reusable photo via Wikimedia Commons';
...
function observePlayerPhotos(){
  ...
  if(!('IntersectionObserver' in window)){
    avatars.slice(0,30).forEach(hydrateAvatar);
    return;
  }
...
async function load(){
  try{
    ...
    status.textContent=payload.partial?`${allPlayers.length} current players loaded • ${withApiArt} API photos • some roster feeds retrying later`:`${allPlayers.length} current players • ${withApiArt} current API photos • Wikimedia fills remaining gaps`;
  }catch(e){
...
async function openProfile(id){
  ...
  modal.showModal();
  try{
    const profileResult=await fetch(`/api/player?id=${encodeURIComponent(id)}&artwork=2`,{headers:{Accept:'application/json'}}).then(async r=>{const payload=await r.json().catch(()=>({}));if(!r.ok)throw new Error(payload.error||'Profile unavailable');return payload;});
    const payload=profileResult,p=payload.player||{},name=p.name||roster?.name||'Player';
    ...
    const sourceNote=media?.resolvedBy==='current-roster-api'
      ? '<p class="media-license-note">Current roster artwork is supplied through TheSportsDB. When its player record carries a Creative Commons tag, that license is shown in the caption; otherwise the provider source and API terms are linked.</p>'
      : media?.image
        ? '<p class="media-license-note">Fallback photo is a reusable Wikimedia Commons image with credit and license details directly beneath it.</p>'
        : '';
